...
def run_benchmark(base_url: str) -> list[dict]:
    ...
    if stats.gpu_temp_c:
        print(f"  GPU:                    {stats.gpu_temp_c:.0f}°C  {stats.gpu_freq_mhz or '?':.0f} MHz")
    print(f"{'─'*65}")
...
def make_recommendation(results: list[dict], ram_total: float) -> dict:
    ...
    safe_by_ram = [
        r for r in valid
        if r.get("ram_free_gb", 0) >= RAM_SAFETY_MARGIN_GB
    ]
    if not safe_by_ram:
...
def test_print_recommendation(benchmark_results):
    ...
    print(f"    TTFT at this setting: {rec['ttft']:.2f}s  "
          f"(baseline: {baseline_ttft:.2f}s)")
...
def main():
    ...
    if baseline_ttft:
        ...
        print(f"    TTFT at this setting: {rec['ttft']:.2f}s  ({ratio:.1f}× baseline {baseline_ttft:.2f}s)")
